...
render();
    ...
    function normalizeItem(item){
      ...
      if(item.rarity==="SECRET"){
        ...
        item.stars="SECRET";
        item.killCount=Number(item.killCount)||0;
        item.maxLevel=Number(item.maxLevel)||5;
        item.level=Math.min(item.maxLevel,Math.floor(item.killCount/10));
        item.bonus=Number(item.bonus)||0;
        item.specialMagic=item.specialMagic||{name:"未設定",damage:0,mpCost:20};
        return item;
      }
      item.rarity=item.rarity||"NORMAL";
      item.killCount=Number(item.killCount)||0;
      ...
      item.bonus=item.stars*item.level;
      return item;
    }
    function load(){
      try{
    ...
    function getEquipmentLevel(item){
      if(!item)return 0;
      return Math.min(Number(item.maxLevel)||5,Math.floor((Number(item.killCount)||0)/10));
    }
...
function getEquipmentMaxHpMpBonus(){
  const weapon=save.equipment?.weapon;
  ...
  if(weapon && weapon.rarity!=="SECRET" && Number(weapon.stars)===10){
    hpBonus+=30;
    mpBonus+=30;
  }

  if(armor && armor.rarity!=="SECRET" && Number(armor.stars)===10){
    hpBonus+=30;
    mpBonus+=30;
  }
  ...
  return getEquipmentLevel(item)*item.stars;
    ...
    function getSpecialName(item){
      if(!item)return null;
      if(item.rarity==="SECRET"){
        return getEquipmentLevel(item)>=Number(item.maxLevel||5)
          ? item.specialMagic?.name||null : null;
      }
      if(getEquipmentLevel(item)<5)return null;
      return EQUIPMENT_SPECIALS[item.stars]||null;
    }
    ...
    function getSpecialDamage(item){
      if(item?.rarity!=="SECRET")return 0;
      return getEquipmentLevel(item)>=Number(item.maxLevel||5)
        ? Number(item.specialMagic?.damage)||0 : 0;
    }

    function getSpecialMpCost(item){
      if(item?.rarity!=="SECRET")return 20;
      return getEquipmentLevel(item)>=Number(item.maxLevel||5)
        ? Number(item.specialMagic?.mpCost)||20 : 20;
    }
    function itemList(){
      const inv=save.inventory||{potion:0,ether:0,items:[]};
      const eq=save.equipment||{weapon:null,armor:null};
      ...
      if(inv.items?.length){
        lines.push(
          ...
          ...inv.items.map((x,i)=>
            `${i+1}. ${x.type==="weapon"?"武器":"防具"} ★${x.stars} ${x.name} Lv.${getEquipmentLevel(x)}/5 `+
            `${x.type==="weapon"?"攻撃補正":"防御補正"}+${getEquipmentBonus(x)} `+
            `${x.rarity==="SECRET"?"SECRET":""} `+
            `${getSpecialName(x)?`スペシャルマジック:${getSpecialName(x)}`:""}`
          )
        );
      }else lines.push("装備品: なし");
      lines.push(`現在の武器: ${eq.weapon?`★${eq.weapon.stars} ${eq.weapon.name}（Lv.${getEquipmentLevel(eq.weapon)}/5 攻撃補正+${getEquipmentBonus(eq.weapon)}）`:"なし"}`);
    ...
    function synthesizeItems(index1,index2,index3){
      const indexes=[Number(index1)-1,Number(index2)-1,Number(index3)-1];
      if(indexes.some(i=>i<0))return usage("/synthesize","3つのアイテム番号を指定してください。");
      if(new Set(indexes).size!==3)return log("同じアイテムを複数回指定することはできません。","error");
      const items=indexes.map(i=>save.inventory.items?.[i]);
      if(items.some(x=>!x))return log("指定されたアイテムがありません。","error");
      if(items.some(x=>x.rarity==="SECRET"))return log("Secret装備は合成素材にできません。","error");
      const rarity=Number(items[0].stars);
      if(items.some(x=>Number(x.stars)!==rarity))return log("同じレアリティの装備3つが必要です。","error");
      if(rarity>=10)return log("★10はこれ以上合成できません。","warn");

      indexes.sort((a,b)=>b-a).forEach(i=>save.inventory.items.splice(i,1));

      if(rarity>=5 && Math.random()<SECRET_SYNTHESIS_CHANCE){
        const keys=Object.keys(SECRET_EQUIPMENT);
        const secret=SECRET_EQUIPMENT[keys[rand(0,keys.length-1)]];
        const item={
          id:String(Date.now()+Math.random()),type:secret.type,rarity:"SECRET",stars:"SECRET",
          name:secret.name,bonus:secret.bonus,killCount:0,level:0,maxLevel:secret.maxLevel,
          specialMagic:{...secret.specialMagic}
        };
        save.inventory.items.push(item);
        persist();
        return log(`✨✨✨ SECRET合成成功！
    ${item.name}
    スペシャルマジック: ${item.specialMagic.name}
    Lv.${item.maxLevel}で解放
    固定ダメージ: ${item.specialMagic.damage}
    MP消費: ${item.specialMagic.mpCost}`,"success");
      }

      const newStars=rarity+1;
      const type=items[0].type;
      const names=type==="weapon"?WEAPON_NAMES_BY_STAR[newStars]:ARMOR_NAMES_BY_STAR[newStars];
      const item={
        id:String(Date.now()+Math.random()),type,rarity:"NORMAL",stars:newStars,
        name:names[rand(0,names.length-1)],bonus:0,killCount:0,level:0
      };
      save.inventory.items.push(item);
      persist();
      log(`🔨 合成成功！
    ★${rarity} × 3
    → ★${newStars} ${item.name}`,"success");
    }
...
c.pow += 2;
    ...
    function increaseEquipmentKills(){
      ...
      for(const item of items){
        ...
        const lv=getEquipmentLevel(item);
        if(item.killCount%10===0){
          log(`⚔ ${item.name} が装備Lv.${lv}/${item.maxLevel||5}になった！`,"success");
        }
        if(lv===(Number(item.maxLevel)||5) && item.killCount%10===0){
          const name=getSpecialName(item);
          if(name)log(`✨ ${item.name} の${item.rarity==="SECRET"?"スペシャルマジック":"必殺技"}「${name}」が解放された！`,"success");
        }
      }
    }
    ...
    function pvpMagic(n){
      ...
      const specialName=getSpecialName(weapon);
      const isSpecial=specialName&&n.trim()===specialName;
      const cost=isSpecial?getSpecialMpCost(weapon):10;
      if(p.mp<cost)return log(`MPが足りません。必要MP: ${cost}`,"error");
      p.mp-=cost;
      const normal=normalDamageAgainst({defense:0},p,getEquipmentBonus(weapon));
      const d=isSpecial&&weapon?.rarity==="SECRET"?getSpecialDamage(weapon):isSpecial?normal*3:Math.max(1,rand(18,35)+p.int);
      t.hp-=d;
      log(isSpecial?`✨ ${p.name}の必殺技「${specialName}」！ ${d}ダメージ！`:`${p.name}の${n} → ${t.name}: ${d}ダメージ`,isSpecial?"success":"system");
      if(t.hp<=0){
        log(`${p.name} の勝利！`,"success");
    ...
    */
    ...
    function onlineStatusKey(room){
      const p1=room.players?.player1,p2=room.players?.player2;
      return [room.status,room.turn,p1?.hp,p1?.mp,p2?.hp,p2?.mp].join("|");
    }
    ...
    function help(){
      log(
        ...
        "★10 ラグナロク\n"+
        "\n【装備合成】\n"+
        "/synthesize 番号 番号 番号  同じレアリティの装備3つを合成。\n"+
        "合成すると1つ上の★になります。★5以上の合成では10%の確率でSecret装備が出ます。\n"+
        "Secret装備は合成素材にできません。\n"+
        "\n【SECRETレアリティ】\n"+
        "Secret装備は最大Lv.まで育てるとスペシャルマジックが解放されます。\n"+
        "スペシャルマジックは固定ダメージで、MP消費も装備ごとに設定できます。\n"+
        "Secret装備を追加するときは、SECRET_EQUIPMENTに名前・ダメージ・MP消費を追加してください。\n"+
        "\n【敵・戦闘】\n"+
        COMMANDS.filter(c=>["/enemy_list","/enemy_info","/battle_start","/battle_status","/battle_attack","/magic_cast","/battle_run"].includes(c[0])).map(c=>`${c[0]} ${c[2]} | ${c[3]}`).join("\n")+
        "\n\n【PvP】\n"+
        "PvP開始時にそれぞれのキャラクターのステータスをコピーします。\n"+
    ...
    function execute(raw){
      ...
      switch(cmd){
        ...
        case"/item_use":return useItem(p[0]);
        case"/equip":return equipItem(p[1]||p[0]);
        case"/synthesize":return synthesizeItems(p[0],p[1],p[2]);
        case"/enemy_list":return enemyList();
        case"/enemy_info":return enemyInfo(p[0],p[1]);
        case"/battle_start":return battleStart(p[0],p[1]);
        case"/battle_status":return battleStatus();
