        ...
        function normalizeItem(item){
          ...
          if(item.rarity==="SECRET"){
            ...
            const savedLevel=Number(item.level);
            const calculatedLevel=Math.floor(item.killCount/10);
            item.level=Number.isFinite(savedLevel)
              ? Math.max(0,savedLevel)
              : calculatedLevel;
          ...
          if(statusEl){
            statusEl.textContent=c
              ? `${c.name} Lv.${c.level} EXP ${c.exp}/${c.nextExp}　HP ${c.hp}/${c.maxHp}　MP ${c.mp}/${c.maxMp}　STR ${c.str}　DEX ${c.dex}　INT ${c.int}　POW ${c.pow}　Gold ${save.gold}`+
                `　武器Lv ${getEquipmentLevel(w)}　防具Lv ${getEquipmentLevel(a)}`
              : "キャラクター未作成";
          }
          renderBattleScene();
        }
        ...
        function attack(){
          ...
          // 確定ドロップ
          if(e.id==="cat" || e.key==="cat" || e.name==="ルナティック猫"){
            ...
            save.inventory ??= {potion:0,ether:0,items:[]};
            save.inventory.items ??= [];
            save.inventory.items.push(drop);
        ...
        */
        ...
        /* =========================================
           白黒ドット絵バトルシーン
           ここで canvas のサイズ・描画・攻撃エフェクトをすべて管理
        ========================================= */
        let battleSceneState = {
          action: null,
          actionUntil: 0,
          actionSide: null,
          lastBattleKey: ""
        ...
        function sceneAction(side){
          battleSceneState.action=side||"player";
          battleSceneState.actionSide=side||"player";
          battleSceneState.actionUntil=performance.now()+420;
          renderBattleScene();
        }

        function pixelRect(ctx,x,y,w,h,fill="#111"){
          ctx.fillStyle=fill;
          ctx.fillRect(Math.round(x),Math.round(y),Math.max(1,Math.round(w)),Math.max(1,Math.round(h)));
        }

        function sceneGauge(ctx,x,y,w,h,value,maxValue){
          ...
          for(let i=0;i<total;i++){
            const filled=i<Math.ceil(ratio*total);
            pixelRect(ctx,x+i*(cellW+gap),y,cellW,h,filled?"#111":"#fff");
            ctx.strokeStyle="#111";
            ctx.strokeRect(
              Math.round(x+i*(cellW+gap)),
              Math.round(y),
        ...
        function sceneNameKind(enemy){
          const id=String(enemy?.id||"").toLowerCase();
          const name=String(enemy?.name||"");

          if(id==="cat"||name.includes("猫"))return "cat";
          if(id.includes("slime")||name.includes("スライム"))return "slime";
          if(id.includes("dragon")||id.includes("wyvern")||name.includes("竜")||name.includes("ワイバーン"))return "dragon";
          if(id.includes("golem")||name.includes("ゴーレム"))return "golem";
          if(id.includes("skeleton")||id.includes("lich")||name.includes("スケルトン")||name.includes("リッチ"))return "skeleton";
          if(id.includes("kraken")||name.includes("クラーケン"))return "kraken";
          if(id.includes("void")||name.includes("虚無"))return "void";
        ...
        function drawPlayer(ctx,x,y,scale){
          const weapon=save.equipment?.weapon;
          const armor=save.equipment?.armor;
          const weaponName=String(weapon?.name||"");
          const armorName=String(armor?.name||"");

          // 頭
          pixelRect(ctx,x-12*scale,y-58*scale,24*scale,24*scale,"#fff");
          ctx.strokeStyle="#111";
          ctx.strokeRect(x-12*scale,y-58*scale,24*scale,24*scale);

          // 防具の見た目
          let body="#fff";
          if(armorName.includes("竜")||armorName.includes("ドラゴン"))body="#111";
          if(armorName.includes("鎧")||armorName.includes("アーマー"))body="#777";
          if(armor?.rarity==="SECRET")body="#111";

          pixelRect(ctx,x-17*scale,y-30*scale,34*scale,42*scale,body);
          ctx.strokeStyle="#111";
          ctx.strokeRect(x-17*scale,y-30*scale,34*scale,42*scale);

          // 腕
          ctx.beginPath();
          ctx.moveTo(x-16*scale,y-20*scale);
          ctx.lineTo(x-35*scale,y+4*scale);
          ctx.moveTo(x+16*scale,y-20*scale);
          ctx.lineTo(x+35*scale,y+4*scale);
          ctx.stroke();

          // 足
          pixelRect(ctx,x-14*scale,y+12*scale,9*scale,30*scale,"#111");
          pixelRect(ctx,x+5*scale,y+12*scale,9*scale,30*scale,"#111");

          // 武器
          const swordX=x+38*scale;
          const swordY=y-2*scale;
          const isAxe=weaponName.includes("斧");
          const isSpear=weaponName.includes("槍");
          const isStaff=weaponName.includes("杖");
          const isSecret=weapon?.rarity==="SECRET";
          ...
          if(isAxe){
            ...
            ctx.lineTo(swordX+8*scale,swordY-22*scale);
            ctx.stroke();
            pixelRect(ctx,swordX+3*scale,swordY-30*scale,22*scale,18*scale,isSecret?"#111":"#fff");
            ctx.strokeRect(swordX+3*scale,swordY-30*scale,22*scale,18*scale);
          }else if(isSpear){
            ctx.beginPath();
            ctx.moveTo(swordX,swordY+30*scale);
            ctx.lineTo(swordX+10*scale,swordY-34*scale);
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(swordX+10*scale,swordY-42*scale);
            ctx.lineTo(swordX+2*scale,swordY-28*scale);
            ctx.lineTo(swordX+18*scale,swordY-28*scale);
            ctx.closePath();
            ctx.stroke();
          }else if(isStaff){
            ctx.beginPath();
            ctx.moveTo(swordX,swordY+30*scale);
            ctx.lineTo(swordX,swordY-35*scale);
            ctx.stroke();
            ctx.strokeRect(swordX-7*scale,swordY-42*scale,14*scale,14*scale);
          }else{
            ctx.beginPath();
            ctx.moveTo(swordX,swordY+30*scale);
            ctx.lineTo(swordX+18*scale,swordY-35*scale);
            ctx.stroke();
            pixelRect(ctx,swordX-5*scale,swordY+24*scale,10*scale,10*scale,"#111");
          }
        }

        function drawEnemy(ctx,x,y,scale,enemy){
          const kind=sceneNameKind(enemy);
          const dark=enemy?.boss||kind==="void";

          if(kind==="slime"){
            ctx.beginPath();
            ctx.moveTo(x-42*scale,y+30*scale);
            ctx.lineTo(x-35*scale,y-5*scale);
            ctx.lineTo(x-20*scale,y-25*scale);
            ctx.lineTo(x+20*scale,y-25*scale);
            ctx.lineTo(x+35*scale,y-5*scale);
            ctx.lineTo(x+42*scale,y+30*scale);
            ctx.closePath();
            ctx.fillStyle="#fff";
            ctx.fill();
            ctx.strokeStyle="#111";
            ...
            pixelRect(ctx,x-28*scale,y-32*scale,56*scale,55*scale,"#fff");
            ctx.strokeStyle="#111";
            ctx.strokeRect(x-28*scale,y-32*scale,56*scale,55*scale);
            ctx.beginPath();
            ctx.moveTo(x-25*scale,y-32*scale);
            ...
            ctx.lineTo(x+8*scale,y-42*scale);
            ctx.stroke();
            pixelRect(ctx,x-15*scale,y-10*scale,5*scale,5*scale,"#111");
            pixelRect(ctx,x+10*scale,y-10*scale,5*scale,5*scale,"#111");
            pixelRect(ctx,x-5*scale,y+8*scale,10*scale,4*scale,"#111");
          }else if(kind==="dragon"||kind==="boss"){
            pixelRect(ctx,x-36*scale,y-45*scale,72*scale,72*scale,dark?"#111":"#fff");
            ctx.strokeStyle="#111";
            ctx.strokeRect(x-36*scale,y-45*scale,72*scale,72*scale);
            ctx.beginPath();
            ctx.moveTo(x-30*scale,y-45*scale);
            ctx.lineTo(x-45*scale,y-70*scale);
            ctx.moveTo(x+30*scale,y-45*scale);
            ctx.lineTo(x+45*scale,y-70*scale);
            ctx.moveTo(x+36*scale,y-5*scale);
            ctx.lineTo(x+75*scale,y+12*scale);
            ctx.stroke();
          }else if(kind==="golem"){
            pixelRect(ctx,x-42*scale,y-55*scale,84*scale,95*scale,"#777");
            ctx.strokeStyle="#111";
            ctx.strokeRect(x-42*scale,y-55*scale,84*scale,95*scale);
            pixelRect(ctx,x-24*scale,y-22*scale,9*scale,9*scale,"#111");
            pixelRect(ctx,x+15*scale,y-22*scale,9*scale,9*scale,"#111");
          }else if(kind==="skeleton"){
            ctx.strokeStyle="#111";
            ctx.lineWidth=Math.max(2,3*scale);
            ctx.strokeRect(x-22*scale,y-55*scale,44*scale,44*scale);
            ctx.beginPath();
            ctx.moveTo(x-30*scale,y-4*scale);
            ctx.lineTo(x+30*scale,y+30*scale);
            ctx.moveTo(x+30*scale,y-4*scale);
            ctx.lineTo(x-30*scale,y+30*scale);
            ctx.stroke();
          }else if(kind==="kraken"){
            pixelRect(ctx,x-30*scale,y-28*scale,60*scale,55*scale,"#fff");
            ctx.strokeStyle="#111";
            ctx.strokeRect(x-30*scale,y-28*scale,60*scale,55*scale);
            for(let i=-2;i<=2;i++){
              ctx.beginPath();
              ctx.moveTo(x+i*12*scale,y+22*scale);
              ctx.lineTo(x+i*20*scale,y+65*scale);
              ctx.stroke();
            }
          }else if(kind==="void"){
            pixelRect(ctx,x-38*scale,y-48*scale,76*scale,95*scale,"#111");
            ctx.strokeStyle="#111";
            ctx.strokeRect(x-38*scale,y-48*scale,76*scale,95*scale);
            for(let i=0;i<5;i++){
              ctx.beginPath();
              ctx.moveTo(x-35*scale+i*17*scale,y-42*scale);
              ctx.lineTo(x-55*scale+i*26*scale,y-70*scale);
              ctx.stroke();
            }
          }else{
            pixelRect(ctx,x-25*scale,y-52*scale,50*scale,52*scale,"#fff");
            ctx.strokeStyle="#111";
            ctx.strokeRect(x-25*scale,y-52*scale,50*scale,52*scale);
            pixelRect(ctx,x-22*scale,y,15*scale,38*scale,"#111");
            pixelRect(ctx,x+7*scale,y,15*scale,38*scale,"#111");
          }
        }

        function drawActionEffect(ctx,w,h,now){
          if(!battleSceneState.actionUntil||now>battleSceneState.actionUntil)return;

          const remain=battleSceneState.actionUntil-now;
          const progress=1-remain/420;
          const side=battleSceneState.actionSide==="enemy"?"enemy":"player";
          ...
          if(side==="player"){
            ...
            const y=h*0.47;
            ctx.beginPath();
            ctx.moveTo(x-80,y+55);
            ctx.lineTo(x+55,y-55);
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(x-65,y+55);
            ctx.lineTo(x+70,y-55);
            ctx.stroke();
          }else{
            const x=w*0.48-progress*80;
            const y=h*0.47;
            ctx.beginPath();
          ...
          for(let i=0;i<10;i++){
            const px=(side==="player"?w*0.62:w*0.38)+Math.sin(i*7+progress*8)*80;
            const py=h*0.47+Math.cos(i*4+progress*10)*50;
            pixelRect(ctx,px,py,4,4,"#111");
          }

          requestAnimationFrame(renderBattleScene);
        }
        ...
        function renderBattleScene(){
          ...
          if(c){
            ...
            ctx.fillText("HP",w*0.12,44);
            sceneGauge(ctx,w*0.17,33,w*0.28,14,c.hp,c.maxHp);
          }
          ...
          if(enemy){
            ...
            ctx.fillText("EHP",w*0.62,44);
            sceneGauge(ctx,w*0.67,33,w*0.28,14,enemy.hp,enemy.maxHp);
          }else{
            ctx.font="bold 16px monospace";
            ctx.fillStyle="#111";
            ctx.fillText("戦闘待機中",w*0.43,h*0.45);
          }

          drawActionEffect(ctx,w,h,now);
        }
        ...
        window.addEventListener("resize",resizeBattleScene);
        resizeBattleScene();
        render();
